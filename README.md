...
Deploy `frontend` to Vercel/Netlify with `NEXT_PUBLIC_API_URL` set to the hosted API. Deploy `backend` to Render/Railway with persistent disk storage for SQLite, `DATABASE_URL`, and `CORS_ORIGINS` set to the frontend origin. For multi-instance production, migrate to PostgreSQL as described in the scaling guide.
